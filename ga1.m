function [x,endPop,bPop,traceInfo] = ga1(bounds,evalFN,evalOps,startPop,opts,...
...
if any(evalFN<48) %Not using a .m file
    ...
    if opts(2) == 1 %Float ga
        e1str = ['[c1(xZomeLength),~]=' evalFN '(c1,inputNum, hiddenNum, outputNum, net, XTrain, TTrain);'];  
        % [c1 c1(xZomeLength)] = evalFN(c1,[gen evalOps]);
        e2str = ['[c2(xZomeLength),~]=' evalFN '(c2,inputNum, hiddenNum, outputNum, net, XTrain, TTrain);'];  
    else %Binary ga
        e1str = ['x=b2f(endPop(j,:),bounds,bits);[v,~]=' evalFN ...
        '(x,inputNum, hiddenNum, outputNum, net, XTrain, TTrain); endPop(j,:)=[f2b(x,bounds,bits) v];'];  
    end
end
...
if isempty(startPop) %Generate a population at random
  %startPop=zeros(80,size(bounds,1)+1);
    startPop = initializega1(80,bounds,evalFN,evalOps,opts(1:2));
end
...
validationFcn1 = @(x) any(validatestring(x, validStrings1, 'ga1','xOverFNs',10)); 
validStrings2 = {'boundaryMutation', 'multiNonUnifMutation', 'nonUnifMutation', 'unifMutation' };
validationFcn2 = @(x) any(validatestring(x, validStrings2, 'ga1','mutFNs',12)); 
