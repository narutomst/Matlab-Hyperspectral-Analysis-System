...
[pd,fa] = hyperRoc(r);
figure; plot(fa,pd,'.'); grid on; 
str = {algorithm;targetFilenames{1}};
t = title(str,'Interpreter','none');
% 由于title默认的'interpreter'为'tex'，所以就会解释地址中的'\'
% 此处需要关闭'Interpreter'属性，同时将标题排列成一个两行的形式
% 数组形式不能用，所以用2×1 cell 来装载标题str
